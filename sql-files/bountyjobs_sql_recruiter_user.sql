...
with rs_name as (SELECT 
		...
		RS.REFERREE_ID
	FROM 
		bounty_jobs.referral_slot RS
		JOIN bounty_jobs.person P2 ON P2.PERSON_ID = RS.REFERRER_ID
		JOIN bounty_jobs.company C ON C.COMPANY_ID = P2.COMPANY_ID AND C.COMPANY_TYPE_CODE = 'E'
)
SELECT
DISTINCT	PT.PARTY_ID,
    ...
    END AS ACTIVE_EMAIL_SUPPRESSED,
	 CASE
        WHEN DATEDIFF(DAY, PN.LAST_LOGIN, CURRENT_DATE) > 90 THEN 
            CASE
                WHEN EXISTS (
                    SELECT 1 
                    FROM bounty_jobs.email_suppression_list ESL
	...
	(CASE WHEN R.APPROVED_REGISTRATION = 'Y' THEN TRUE ELSE FALSE END) AS APPROVED_REGISTRATION, CASE 
        WHEN EXISTS (
            SELECT 1
            FROM bounty_jobs.party_role PRIN
    ...
    WHEN EXISTS (
        SELECT 1
        FROM bounty_jobs.company_diversity_status CDS
...
FROM 
	bounty_jobs.party PT
	JOIN bounty_jobs.person PN ON PT.PARTY_ID = PN.PERSON_ID 
	JOIN bounty_jobs.recruiter R ON PN.PERSON_ID = R.PERSON_ID
	JOIN bounty_jobs.company C ON C.COMPANY_ID = PN.COMPANY_ID
	left join rs_name rn on rn.REFERREE_ID=PN.PERSON_ID 
	LEFT JOIN bounty_jobs.relationship REL ON REL.SOURCE_PARTY_ID = PN.PERSON_ID AND REL.TARGET_PARTY_ID = PN.COMPANY_ID AND REL.RELATIONSHIP_TYPE_CODE IN ('CA') AND REL.RECORD_STATUS_CODE = 'A' 
	LEFT JOIN bounty_jobs.firm_size FS ON FS.ID = R.FIRM_SIZE_ID
	...
	LEFT JOIN bounty_jobs.education_level EL ON EL.CODE = R.EDUCATION_LEVEL_CODE
	LEFT JOIN bounty_jobs.company_size CS ON CS.ID = R.FIRM_SIZE_ID
	LEFT JOIN bounty_jobs.recruiter_ats_type RAT ON RAT.CODE = R.ATS
	LEFT JOIN 	(
				SELECT 
					AIN.*
				FROM 
	...
	LEFT JOIN bounty_jobs.referral_slot RS on PN.PERSON_ID = RS.REFERREE_ID
	LEFT JOIN bounty_jobs.person REF ON RS.REFERRER_ID = REF.PERSON_ID
	...
	LEFT JOIN bounty_jobs.certification C1 ON C1.CODE = R.CERTIFICATION_CODE_1
	LEFT JOIN bounty_jobs.certification C2 ON C2.CODE = R.CERTIFICATION_CODE_2
	LEFT JOIN bounty_jobs.certification C3 ON C3.CODE = R.CERTIFICATION_CODE_3
    LEFT JOIN bounty_jobs.industry I1 ON R.INDUSTRY_ID_1 = I1.ID
    LEFT JOIN bounty_jobs.industry I2 ON R.INDUSTRY_ID_2 = I2.ID
    LEFT JOIN bounty_jobs.industry I3 ON R.INDUSTRY_ID_3 = I3.ID 
    LEFT JOIN bounty_jobs.job_category J1 ON R.JOB_CATEGORY_ID_1 = J1.ID
    LEFT JOIN bounty_jobs.job_category J2 ON R.JOB_CATEGORY_ID_2 = J2.ID
    LEFT JOIN bounty_jobs.job_category J3 ON R.JOB_CATEGORY_ID_3 = J3.ID 