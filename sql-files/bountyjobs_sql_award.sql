-- select * from bounty.REGION_ADMIN1
-- select * from bounty.REGION_ADMIN2
...
SELECT 
    ...
    (SELECT MT.BODY_TEXT FROM bounty_jobs.message_text MT WHERE MT.ID = HMM.MESSAGE_ID) AS MESSAGE,
    ...
    (case when HM.CURRENCY_CODE = 'USD' then 1 else (SELECT CR.VALUE FROM bounty_jobs.conversion_rate CR
	  ...
	  (SELECT SUM(CASE WHEN A.ACCOUNT_TYPE_CODE = 'D' THEN AE.AMOUNT ELSE -AE.AMOUNT END)
       FROM bounty_jobs.transaction T
                JOIN bounty_jobs.account_entry AE ON AE.TRANSACTION_ID = T.ID
                JOIN bounty_jobs.account A ON A.ID = AE.ACCOUNT_ID
           AND A.ACCOUNT_CHART_TYPE_ID IN (1, 2, 4)
                JOIN bounty_jobs.company C ON C.COMPANY_ID = A.COMPANY_ID
      ...
      (case when HM.HIRE_DATE > CURRENT_DATE then 
      (case when HM.CURRENCY_CODE = 'USD' then COALESCE(JP.POSTING_EFFECTIVE_DATE, JP.CREATION_DATE) else (SELECT CR.EFFECTIVE_DATE FROM bounty_jobs.conversion_rate CR
              WHERE CR.SOURCE_CURRENCY_CODE = UPPER(TRIM(HM.CURRENCY_CODE))
                AND CR.TARGET_CURRENCY_CODE = UPPER(TRIM('USD'))
                AND CR.EFFECTIVE_DATE <= COALESCE(JP.POSTING_EFFECTIVE_DATE, JP.CREATION_DATE)
              ORDER BY CR.EFFECTIVE_DATE DESC LIMIT 1) end)
	  ...
	  cast(COALESCE(BJ_AMOUNT.AMOUNT_SUM,
          ...
          (case when JP.RETAINED_FLAG='Y' then (
              SELECT -- subtract out what was already paid
                     SUM(AMOUNT)
              FROM bounty_jobs.account_entry AE
                       JOIN bounty_jobs.transaction T ON AE.TRANSACTION_ID = T.ID AND TRANSACTION_TYPE_ID=35
                       JOIN bounty_jobs.account A ON AE.ACCOUNT_ID = A.ID
                       JOIN bounty_jobs.account_chart_type ACT ON A.ACCOUNT_CHART_TYPE_ID = ACT.ID
    ...
    round(COALESCE(BJ_AMOUNT.AMOUNT_SUM,
         ...
         * CAST(
              ...
              (case when HM.HIRE_DATE > CURRENT_DATE then 
                (case when HM.CURRENCY_CODE = 'USD' then 1 else 
                      (SELECT CR.VALUE FROM bounty_jobs.conversion_rate CR
                       WHERE CR.SOURCE_CURRENCY_CODE = UPPER(TRIM(HM.CURRENCY_CODE))
                         AND CR.TARGET_CURRENCY_CODE = UPPER(TRIM('USD'))
                         AND CR.EFFECTIVE_DATE <= COALESCE(JP.POSTING_EFFECTIVE_DATE, JP.CREATION_DATE)
                       ORDER BY CR.EFFECTIVE_DATE DESC LIMIT 1) end)
                ...
                WHEN HM.HIRE_DATE > CURRENT_DATE THEN 
                  (
                    CASE 
                      ...
                      ELSE (
                        SELECT CR.VALUE 
                        FROM bounty_jobs.conversion_rate CR
                        WHERE CR.SOURCE_CURRENCY_CODE = UPPER(TRIM(HM.CURRENCY_CODE))
  ...
  COALESCE(
    ...
    *
    ...
    (
      ...
      FROM bounty_jobs.submission_award_question_answer SAQA
      JOIN bounty_jobs.company_award_question CAQ
        ON CAQ.AWARD_QUESTION_ID = SAQA.AWARD_QUESTION_ID
      JOIN bounty_jobs.award_question AQ
...
FROM bounty_jobs.hire_message HM
         JOIN bounty_jobs.message HMM ON HM.MESSAGE_ID = HMM.MESSAGE_ID
         JOIN bounty_jobs.submission S ON HM.SUBMISSION_ID = S.SUBMISSION_ID
         JOIN bounty_jobs.job_posting JP ON S.JOB_POSTING_ID = JP.JOB_POSTING_ID AND JP.WORKFLOW_HANDLER_NAME NOT IN ('ContractPosting','AtsManualContractPosting')
         JOIN bounty_jobs.company HM_COMP ON JP.COMPANY_ID = HM_COMP.COMPANY_ID
         JOIN bounty_jobs.candidate C ON S.CANDIDATE_ID = C.CANDIDATE_ID
         JOIN bounty_jobs.recruiter REC ON C.RECRUITER_PERSON_ID = REC.PERSON_ID
         JOIN bounty_jobs.person ORIG_REC ON REC.PERSON_ID = ORIG_REC.PERSON_ID
         JOIN bounty_jobs.transaction_cost TC ON (case when JP.RETAINED_FLAG='Y' then TC.LOOKUP_KEY='BOUNTYFEEHRET' AND TC.TRANSACTION_TYPE_ID=36 else TC.LOOKUP_KEY='BOUNTYFEEH' AND TC.TRANSACTION_TYPE_ID=6 end)
         LEFT JOIN (
    SELECT
        MAX(T.ID) AS ID,
    ...
    GROUP BY T.SUBMISSION_ID,t.transaction_type_id
         ...
         LEFT JOIN bounty_jobs.contract CT ON
        ...
        AND S.CREATION_DATE <= CT.END_DATE
         LEFT JOIN bounty_jobs.company_transaction_cost CTC ON CTC.TRANSACTION_COST_ID = TC.ID AND CT.CONTRACT_ID = CTC.CONTRACT_ID
         LEFT JOIN bounty_jobs.message HRM ON HRM.PARENT_MESSAGE_ID=HMM.MESSAGE_ID
         LEFT JOIN bounty_jobs.hire_response HR ON HRM.MESSAGE_ID=HR.MESSAGE_ID
         LEFT JOIN bounty_jobs.hire_response_type HRT ON HR.HIRE_RESPONSE_TYPE_CODE = HRT.CODE
         LEFT JOIN (
    SELECT AE.TRANSACTION_ID,
           SUM(case when A.ACCOUNT_TYPE_CODE='C' then AE.AMOUNT else -AE.AMOUNT end) AS AMOUNT_SUM
    FROM bounty_jobs.account_entry AE
             JOIN bounty_jobs.account A ON AE.ACCOUNT_ID = A.ID
             JOIN bounty_jobs.account_chart_type T ON A.ACCOUNT_CHART_TYPE_ID = T.ID
        AND T.LOOKUP_ID = 4200 -- Bounty revenue
             JOIN bounty_jobs.company C ON A.COMPANY_ID = C.COMPANY_ID AND C.COMPANY_TYPE_CODE='E'
    ...
    FROM bounty_jobs.account_entry AE
             JOIN bounty_jobs.account A ON AE.ACCOUNT_ID = A.ID
             JOIN bounty_jobs.company C ON A.COMPANY_ID = C.COMPANY_ID AND C.COMPANY_TYPE_CODE='R'
    GROUP BY TRANSACTION_ID
) HH_AMOUNT ON HH_AMOUNT.TRANSACTION_ID=HIRE_TRANSACTION.ID AND HRT.CODE='A'
         LEFT JOIN bounty_jobs.canadian_invoicing_response CIR ON S.SUBMISSION_ID = CIR.SUBMISSION_ID
         LEFT JOIN bounty_jobs.canadian_tax_response CTR ON CTR.CANADIAN_INVOICING_RESPONSE_ID = CIR.ID